...
public class RadarViewGroup extends ViewGroup implements RadarView.IScanningListener {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        //放置雷达图中需要展示的item圆点
        for (int i = 0; i < childCount; i++) {   //修改i的值
            final int j = i;
    ...
    public void setDatas(SparseArray<Info> mDatas) {
        ...
        for (int i = 0; i < dataLength; i++) {
            CircleView circleView = new CircleView(getContext());
             ...
             circleView.setPaintColor(getResources().getColor(R.color.fuchsia));
            if (mDatas.get(i).getKind().equals("router")) {
                circleView.setPaintColor(getResources().getColor(R.color.fuchsia));
            } else if(mDatas.get(i).getKind().equals("user")) {
                circleView.setPaintColor(Color.BLUE);
            }
    ...
    private void startAnim(CircleView object, int position) {
        if (object != null) {
       try{     object.setPortraitIcon(mDatas.get(position).getPortraitId());
            ObjectAnimator.ofFloat(object, "scaleX", 2f).setDuration(300).start();
            ObjectAnimator.ofFloat(object, "scaleY", 2f).setDuration(300).start();}catch (Exception e){}
        }
    }
